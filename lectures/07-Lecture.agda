...
transsym x y z x≡y z≡y = let open Eq.≡-Reasoning in begin
  x ≡⟨ x≡y ⟩
  y ≡⟨ z≡y ⟨
  z ∎
...
subtrees (leaf a) = self ∷ []
subtrees (node l r)
  = let ls = subtrees l in
    let rs = subtrees r in
    self ∷ {!!} ++ {!!}
...
subtreesAcc :
  ...
  (embed : Subtree t → B) →
  List B
subtreesAcc (leaf a) embed = embed self ∷ []
subtreesAcc (node l r) embed
  = let ls = subtreesAcc l (embed ∘ left) in
    let rs = subtreesAcc r (embed ∘ right) in
    embed self ∷ ls ++ rs
